...
public class AndroidDevice {
	...
	public static final int ANDROID_DEVICE_STATE_CONNECT = 1;
	public static final int ANDROID_DEVICE_STATE_DISCONNECT = 2;
	...
	public StringBuffer adbShell(String cmd) {
		String adbCmd = "adb -s " + deviceID + " shell \"" + cmd + "\"";
		
		HLog.il("command is : " + adbCmd);
		return CommandLine.execCommandAndGetResult(adbCmd);
	}
	...
	public InputStream getPhoneEventInputStream() {
		...
		try {
			logcatProcess = Runtime.getRuntime().exec(cmdString);
		} catch (IOException e) {
			HLog.el(e);
			return null;
		}
	...
	public InputStream getLogcatInputStream() {
		String cmdString = "adb -s " + deviceID + " shell \"logcat\"";
		HLog.il("AndroidDevice.getPhoneEventInputStream(): cmd is:" + cmdString);
		if (logcatProcess != null && logcatProcess.isAlive()) {
			try {
				logcatProcess.getInputStream().close();
